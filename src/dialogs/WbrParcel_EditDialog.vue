...
import {
  wbrRegisterColumnTitles,
  wbrRegisterColumnTooltips
} from '@/helpers/wbr.register.mapping.js'
...
async function onLookup(values) {
  if (
    ...
    runningAction.value ||
    currentParcelId.value != values.id
  )
    return
  runningAction.value = true
  try {
      ...
      <div class="form-section">
        <div class="form-row">
          ...
          <WbrFormField
            ...
            :fullWidth="false"
          />
          <WbrFormField name="currency" :errors="errors" :fullWidth="false" readonly />
        </div>
        <div class="form-row">
          <WbrFormField
            name="recipientName"
          ...
          <PassportNumberWithActions
            :label="wbrRegisterColumnTitles.passportNumber"
            :errors="errors"
