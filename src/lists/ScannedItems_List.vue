...
const { triggerLoad, stop: stopFilterSync } = useDebouncedFilterSync({
  filters: [{ local: localSearch, store: scanneditems_search }],
  ...
  <div class="settings table-3">
    ...
    <div>
      <v-text-field
        ...
        label="Поиск по отсканированным элементам"
        variant="solo"
        hide-details
    ...
    <div v-if="scannedItemsError" class="text-center m-5">
      <div class="text-danger">Ошибка при загрузке результатов сканирования: {{ scannedItemsError }}</div>
    </div>
