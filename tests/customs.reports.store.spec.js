...
describe('customs.reports.store', () => {
  ...
  it('initializes with default state', () => {
    ...
    expect(store.rowsTotalCount).toBe(0)
  ...
  it('fetches report rows using fetchWrapper.get', async () => {
    ...
    expect(store.rowsTotalCount).toBe(2)
    expect(store.rowsHasNextPage).toBe(false)
    expect(store.rowsHasPreviousPage).toBe(false)
    expect(store.loading).toBe(false)
    expect(store.error).toBeNull()
  })
