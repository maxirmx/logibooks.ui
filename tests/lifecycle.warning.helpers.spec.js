...
describe('lifecycle warning helpers', () => {
  ...
  it('uses the requested passport form', () => {
    expect(buildRestartPassportCheckWarning(1)).toBe(
      'Вы запрашиваете проверку паспорта, хотя для данного реестра уже выполнялась операция «Завершить проверку паспортов». Перед выгрузкой накладных или реестра необходимо «Завершить проверку паспортов» ещё раз.'
    )
    expect(buildRestartPassportCheckWarning(2)).toBe(
      'Вы запрашиваете проверку паспортов, хотя для данного реестра уже выполнялась операция «Завершить проверку паспортов». Перед выгрузкой накладных или реестра необходимо «Завершить проверку паспортов» ещё раз.'
    )
  })

  it('builds the clear passport status warning', () => {
    expect(buildClearPassportCheckWarning()).toBe(
      'Вы очищаете статус проверки паспорта, хотя для данного реестра уже выполнялась операция «Завершить проверку паспортов». Перед выгрузкой накладных или реестра необходимо «Завершить проверку паспортов» ещё раз.'
    )
  })
